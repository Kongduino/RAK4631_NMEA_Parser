...
uint32_t RF_FREQUENCY = 868125000; // Hz
int8_t TX_OUTPUT_POWER = 22; // dBm
uint32_t LORA_BANDWIDTH = 0; // [0: 125 kHz, 1: 250 kHz, 2: 500 kHz, 3: Reserved]
uint32_t LORA_SPREADING_FACTOR = 12; // [SF7..SF12]
...
void sendUpdate() {
  ...
  uint16_t len = strlen(msgBuf);
  Serial.println("Sending packet! " + String(len));
  hexDump((unsigned char *)msgBuf, len);
  if (g_BleUartConnected) g_BleUart.println("Sending packet!");
  ...
  if (buff[0] == '/') {
    ...
    if (strcmp("/sf ", cmd) == 0) {
      ...
      if (value < 860.0 || value > 999.0) {
        ...
        g_BleUart.print(buff);
        // do it fo' real
        RF_FREQUENCY = (uint32_t)(value * 1e6);
        SetupRadio();
      }
    }
  }
